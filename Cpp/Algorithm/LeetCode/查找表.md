...
外层加上 `while (true)` 使得不断计算 Sum 的值, 同时使用查找表记录结果 sum 是否曾经得到过, 如果曾经得到过, 那么就可能出现无限循环的问题, 这样就没有办法得到 1.
    ...
    bool wordPattern(string pattern, string str) {
        ...
        for (const auto &c : pattern) {
            if (ss.eof())
                return false;
            ss >> word; 
          	// 考虑两种情况: 1. 如果模式已存在在 record, 要保证
          	// record[c] 与 word 相同;
          	// 2. 如果模式不在 record 中, 那么还要保证对应的 word
          	// 不能和其他模式的 word 相同.
            if (record.count(c) && record[c] != word)
                return false;
            if (!record.count(c) && strset.count(word))
                return false;
...
上面的代码还是有点不漂亮, 当时我在考虑 next 时, 总想怎么才能 continue, 实际有下面更好的办法:
...
思路: 这道题要使用 Trie 树才能获得 O(n) 的结果. 这里先使用哈希表求解. 在哈希表中存储所有的 root, 当访问 sentence 中每一个 word 时, 判断 word 的 prefix 是否存在, 如果存在的话, 就用这个 prefix 来替换 word. 注意下面代码关于返回结果中每个 word 之间空格的处理.
...
上面的解法是我看了 leetcode 的 Solution 进行修改的, 我开始的写法如下, 比上面的方法快一点:
...
leetcode 的解法: https://leetcode.com/articles/replace-words/
