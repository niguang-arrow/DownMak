...
namespace mysort {
    ...
    template <typename T>
    ListNode<T>* bucket_insert_1(ListNode<T> *head, int val) {
    ...
    template <typename T>
    ListNode<T>* bucket_merge_1(ListNode<T> *l1, ListNode<T> *l2) {
        auto dummy = new ListNode<T>(0);
        auto ptr = dummy;
        while (l1 && l2) {
            if (l1->val < l2->val) {
    ...
    template <typename T>
    string bucket_sort_1(vector<T> &arr) {
        int n = arr.size();
        ...
        for (int i = 0; i < n; ++i) {
            ...
            buckets[idx] = bucket_insert_1(head, arr[i]);
        }
        ...
        for (int i = 0; i < BUCKET_NUM; ++i) {
            ...
            ptr->next = bucket_merge_1(ptr->next, buckets[i]);
            ptr = ptr->next;
        }
    ...
    template <typename T>
    void bucket_merge(vector<T> &res, int m, vector<int> &arr) {
        int n = arr.size();
        int k = m + n - 1;
        while (m && n) {
            if (res[m] < arr[n]) res[k--] = arr[n--];
            else res[k--] = res[m--];
        }
        while (n) res[k--] = arr[n--];
        while (m) res[k--] = res[m--];
    ...
    string bucket_sort(vector<T> &arr) {
        ...
        //vector<ListNode<T>*> buckets(BUCKET_NUM, nullptr);
        vector<vector<T>> buckets(BUCKET_NUM, vector<T>{});
        for (int i = 0; i < n; ++i) {
            int idx = i % BUCKET_NUM;
            auto head = buckets[idx];
            head.push_back(arr[i]);
        }


        vector<int> tmp(arr.size(), INT32_MAX);
        int k = 0;
        for (int i = 0; i < BUCKET_NUM; ++i) {
            if (!buckets[i].empty()) {
                // 每个桶中的元素再继续排序
                std::sort(buckets[i].begin(), buckets[i].end());
                bucket_merge(tmp, k, buckets[i]);
                k += buckets.size();
            }
        }
...
int main() {
    ...
    test_sort<int>(mysort::bucket_sort_1<int>, n);
