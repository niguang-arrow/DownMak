...
\begin{tikzpicture}[remember picture,overlay]
	...
	\tNode{job}{([shift={(3.5, \joby)}]current page.north west)}{\MyJob{\job}};
	\node(photo) at ([shift={(-2.7, \photoy)}]current page.north east)			
	{\includegraphics[width=2.2cm,height=3.2cm]{\photo}};
	\node[fit=(photo),draw=headings,rectangle,inner sep=-2pt,shift={(-.02, .01)}] {};
...
\begin{itemize}
\item 专业：信号与信息处理 \hspace*{3cm} 学院：电子信息学院 \hspace*{2cm} 工学硕士 
...
\begin{itemize}
\item 专业：通信工程 \hspace*{4.15cm} 学院：电子信息学院 \hspace*{2cm} 工学学士 
\end{itemize}%
}
