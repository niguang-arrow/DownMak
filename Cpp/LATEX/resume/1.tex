...
\newcommand{\job}{\hspace*{-6cm}算法工程师\\[-.3cm] {\hspace*{1.86cm}\zihao{-4}计算机视觉方向}}
...
构建深度卷积神经网络对带噪图像进行残差学习，算法使用 PyTorch 深度学习框架\\ 实现，利用残差网络、ReLU 等技术加速算法收敛并提升去噪效果，在标准测试图\\ 像上的峰值信噪比达到 state-of-the-art 的结果。预计投递 \textit{Signal Processing}（SCI \\ 检索， IF：3.47）,  关于该算法的报告可查看：\href{https://github.com/zmlhome/Amaze/blob/master/Layer/zml-annual-meeting-20180125.pdf}{2018 年会报告}。
...
\begin{itemize}
	...
	\item 深度学习框架：了解 \textbf{PyTorch}，\textbf{Caffe} 等常用深度学习框架，熟悉深度学习常用的网络结构，如 \textbf{ResNet}，\textbf{VGG}等；熟悉目标检测算法 \textbf{Faster-RCNN}。%掌握网络构建、\textbf{反向传播机制}、常用\textbf{优化算法}，
	\item 理论知识：扎实的计算机基础，了解\textbf{操作系统}、常用\textbf{数据结构}，掌握 C++ 面向对象程序设计思想，熟悉 STL 中各种容器和算法的使用，了解 STL 的底层实现。
